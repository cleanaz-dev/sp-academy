...
export async function handleGameSchema(task: SystemTask, body: any) {
  try {
    ...
    if (status === "COMPLETED") {
      ...
      await prisma.$transaction([
        ...
        prisma.systemTask.update({
          ...
          data: {
            status: "COMPLETED",
            result: result, // Optional: save the output directly on the task too
          },
        }),
      ]);

      return NextResponse.json({ success: true, message: "Schema saved successfully" }, { status: 200 });
    } 
    ...
    if (status === "FAILED") {
      ...
      return NextResponse.json({ success: false, message: "Task marked as failed" }, { status: 200 }); 
