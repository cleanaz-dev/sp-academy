...
export default async function BookReportCard({ book, bookReports }) {
  ...
  // Find the corresponding book report and get its reading logs
  const bookReport = bookReports.find((report) => report.bookId === bookId);
  const readingLogs = bookReport
    ? await getReadingLogsByBookReportId(bookReport.id)
    : [];
