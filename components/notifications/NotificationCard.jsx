...
const NotificationItem = ({ notification, markAsRead }) => {
  ...
  return (
    <div
      className={cn(
        "my-2 cursor-pointer rounded-lg px-4 py-3 transition-colors",
      ...
      <div className="flex items-start gap-3 px-3">
        ...
        <div className="flex flex-1 items-start justify-between gap-3">
          <div>
            ...
            <CardTitle className="mb-1 text-sm font-semibold text-gray-900">
...
export default function NotificationCard({ handleNotificationClick }) {
  ...
  const markAllAsRead = async () => {
    try {
      const response = await fetch(
        "/api/shared-activity/notifications/read-all",
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
        },
